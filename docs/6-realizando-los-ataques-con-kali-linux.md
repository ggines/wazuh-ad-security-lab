...
- **Reconocimiento:** Escaneo de puertos e identificación de servicios con nmap (``nmap -sV -O IP``) hacia Windows Server
  ...
  - **Eventos en Wazuh**: Suricata detectó el escaneo de Nmap y la detección del SO (Firmas ET SCAN Nmap Scripting Engine User-Agent Detected
    (Nmap Scripting Engine) y ET SCAN NMAP OS Detection Probe)
...
- **Ataque de Kerberoasting:** Consiste en hacer que un usuario válido del dominio solicite un ticket Kerberos para una cuenta de servicio (como una cuenta que
...
- **Ataque de AS-REP Roasting:** Este ataque va dirigido a cuentas con preautenticación de Kerberos deshabilitada. Para realizarlo, he creado un
  usuario llamado **'cuenta.rrhh'** con una contraseña débil y con las opciones **'La contraseña nunca expira'** y **'No pedir la autenticación Kerberos previa'** marcadas:
  <p>
